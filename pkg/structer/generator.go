...
type TypeGenerator interface {
    Generate(input io.Reader, implName, pkgName string) ([]byte, error)
}
...
func (d typeGenerator) Generate(input io.Reader, structName, pkgName string) ([]byte, error) {
    return d.generate(input, structName, pkgName)
}
